...
public class ServiceQualityController {
    ...
    public Result listAll(@PathVariable String name) {
        ...
        List<ServiceQuality> result = list.stream().limit(50).collect(Collectors.toList());
    ...
    @PostMapping("/save")
    public Result save(@RequestBody List<ServiceQuality> vos) {

        if(CollectionUtils.isEmpty(vos)){
            return new Result().fail("态势告警，参数为空", 500);
        }

        ArrayList<ServiceQuality> successes = new ArrayList<>();
        ArrayList<ServiceQuality> fails = new ArrayList<>();

        vos.stream().forEach(v -> {
            boolean success = serviceQualityServiceImpl.save(v);
            if(success) {
                successes.add(v);
            }else {
                fails.add(v);
        ...
        HashMap<String, List<ServiceQuality>> result = new HashMap<>();
